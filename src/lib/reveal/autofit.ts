 ...
 */
export function autoFitSlides(revealEl: HTMLElement): void {
    const slidesContainer = revealEl.querySelector<HTMLElement>('.slides');
    if (!slidesContainer) return;

    const slideHeight = slidesContainer.offsetHeight || 700;

    const sections = revealEl.querySelectorAll<HTMLElement>('.slides section');
    ...
    for (const section of sections) {
        ...
        if (contentHeight <= slideHeight + 2 || slideHeight === 0) continue;
