...
describe('layoutGanttRows', () => {
    ...
    it('draws the whole group span on the header bar', () => {
        const { rows } = layoutGanttRows(buildGanttTree(items, true), layoutOpts([], { summaryBar: true }));
    ...
    it('moves labels onto the bars only inside a collapsed group', () => {
        const expanded = layoutGanttRows(buildGanttTree(items, true), layoutOpts([], { summaryBar: true }));
...
describe('buildGanttTooltipModel', () => {
    ...
    it('gives a milestone a date but no duration', () => {
        const model = buildGanttTooltipModel(bar({ key: 't:2', label: 'Ship', taskIndex: 2, covers: [2], milestone: true, endMs: jan(21) }), tasks, edges);
    ...
    it('drops a roll-up bar edges internal to its group, keeping the crossing ones', () => {
        const model = buildGanttTooltipModel(bar({ key: 'g:X:roll', label: 'Phase 1', taskIndex: -1, covers: [0, 1], summary: true }), tasks, edges);
        expect(model.predecessors).toEqual([]);
        expect(model.successors).toEqual(['Ship']);
    });
    it('leaves a roll-up bar without a comment: no single task wrote one', () => {
        const model = buildGanttTooltipModel(bar({ taskIndex: -1, covers: [0, 1], summary: true }), tasks, edges);
...
describe('ganttTooltipText', () => {
    ...
    it('gives a milestone a single date', () => {
        const text = ganttTooltipText({ ...model, milestone: true, days: undefined, endMs: model.startMs }, { locale: 'en', labels });
        expect(text).not.toContain('–');
    });
});
describe('estimateGanttTooltipHeight', () => {
    ...
    it('grows with a longer comment', () => {
        ...
        const long = estimateGanttTooltipHeight({ ...base, comment: 'ok '.repeat(80) }, metrics);
