...
import { extractSlideStem, parseNumericPrefix, compareEntries, createMarkdownSlides } from './slides';
describe('slides utility logic', () => {
    ...
    describe('compareEntries', () => {
        it('sorts by numeric order', () => {
            const a = { order: '01-a', type: 'component', id: 'a' };
            const b = { order: '02-b', type: 'markdown', id: 'b' };
            expect(compareEntries(a, b)).toBeLessThan(0);
        });
        it('sorts components before markdown on same order', () => {
            const a = { order: 'same', type: 'component', id: 'comp' };
            const b = { order: 'same', type: 'markdown', id: 'md' };
            expect(compareEntries(a, b)).toBeLessThan(0);
        });
    });
