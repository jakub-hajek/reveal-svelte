 ...
 * diamond.
 */
function packingFootprint(bar, geometry, o) {
 ...
 */
function clearMilestones(bars, geometry) {
...
export function layoutGanttRows(tree, o) {
    ...
    const walk = (nodes, depth) => {
        for (const node of nodes) {
            ...
            const geometry = clearMilestones(bars, bars.map((bar) => barGeometry(bar, o)));
            const footprints = bars.map((bar, i) => extentOf(bar, geometry[i], NO_LABEL, o.plotWidth));
            const { lanes, laneCount: packedLanes } = packGanttLanes(bars.map((bar, i) => packingFootprint(bar, geometry[i], o)));
...
export function placeGanttMarkerLabelRows(markers, fontSize) {
    ...
    const rowEdges = [];
    const rows = new Array(markers.length).fill(0);
 ...
 */
export function placeGanttTooltip(anchor, 
 ...
 * anyone reads.
 */
export function ganttDurationDays(startMs, endMs) {
 ...
 */
export function formatGanttDuration(days, locale) {
    try {
...
export function ganttTooltipLabels(locale) {
...
function depLabels(edges, tasks, covers, near) {
    const far = near === 'toTask' ? 'fromTask' : 'toTask';
    const seen = new Set();
    const out = [];
    for (const edge of edges) {
        // edges wholly inside this bar describe nothing the reader can see
        if (!covers.has(edge[near]) || covers.has(edge[far]))
            continue;
        if (seen.has(edge[far]))
            continue;
        seen.add(edge[far]);
 ...
 */
export function buildGanttTooltipModel(bar, tasks, edges) {
 ...
 */
export function ganttTooltipText(model, opts) {
    ...
    if (model.days != null)
        parts.push(formatGanttDuration(model.days, locale));
    if (model.progress != null)
        parts.push(`${clamp(Math.round(model.progress), 0, 100)}%`);
    if (model.predecessors.length)
        parts.push(`${labels.dependsOn} ${model.predecessors.join(', ')}`);
    if (model.successors.length)
        parts.push(`${labels.followedBy} ${model.successors.join(', ')}`);
...
function wrappedLines(text, fontSize, innerWidth, bold = false) {
    if (!text || innerWidth <= 0)
        return 1;
    // authored newlines are hard breaks; each resulting line then wraps on its own
    return text
        .split('\n')
        .reduce((sum, line) => sum + Math.max(1, Math.ceil(estimateTextWidth(line, fontSize, { bold }) / innerWidth)), 0);
}
/**
 ...
 */
export function estimateGanttTooltipHeight(model, metrics) {
    ...
    if (model.progress != null)
        blocks.push(metrics.progressHeight);
    if (model.predecessors.length) {
    ...
    if (model.comment)
        blocks.push(wrappedLines(model.comment, fontSize, innerWidth) * lineHeight);
