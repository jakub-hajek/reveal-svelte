...
describe('GanttChart component', () => {
    ...
    it('applies per-marker color and line style', () => {
        ...
        const style = container.querySelector('.gantt-marker')?.style;
    ...
    it('grows the axis only when a marker carries a caption', () => {
        const axisHeight = (root) => root.querySelector('.gantt-body')?.style.getPropertyValue('--gantt-axis-h');
        const bare = render(GanttChart, { props: { tasks, markers: [{ date: '2026-02-01' }] } });
        expect(axisHeight(bare.container)).toBe('26px');
        const labelled = render(GanttChart, {
            props: { tasks, markers: [{ date: '2026-02-01', label: 'Kickoff' }] }
        });
        expect(axisHeight(labelled.container)).toBe('44px');
    });
    it('drops a colliding marker caption to a second line instead of overlapping', () => {
        ...
        expect(axisHeight(container)).toBe('62px');
    });
    it('keeps well-separated marker captions on the same line', () => {
        ...
        expect(axisHeight(container)).toBe('44px');
    });
...
describe('GanttChart detail popup', () => {
    ...
    const bars = (container) => [...container.querySelectorAll('.gantt-bar')];
    const tipOf = (container) => container.querySelector('.gantt-tooltip');
    ...
    it('keeps the newlines the author wrote in a comment', async () => {
        const { container } = render(GanttChart, { props: { tasks: detailed } });
        await fireEvent.focus(bars(container)[1]);
        expect(container.querySelector('.gantt-tooltip-comment')?.textContent).toBe('Blocked on\nthe vendor SDK');
    });
    it('lists dependencies in both directions', async () => {
        const { container } = render(GanttChart, { props: { tasks: detailed, locale: 'en' } });
        await fireEvent.focus(bars(container)[1]);
        const deps = [...container.querySelectorAll('.gantt-tooltip-deps')].map((el) => el.textContent?.replace(/\s+/g, ' ').trim());
    ...
    it('gives a group roll-up its section name and no comment', async () => {
        ...
        const roll = container.querySelector('.gantt-bar.is-summary');
